// Wall display (Q34): display tokens, and the anonymous snapshot RPC.
...
export async function fetchDisplaySnapshot(token: string): Promise<DisplaySnapshot> {
  ...
  if (error) throw error
  const raw = data as unknown as { owner: DisplaySnapshot['owner']; items: BoardItem[]; boards: RawBoard[] }
  return {
    owner: raw.owner,
