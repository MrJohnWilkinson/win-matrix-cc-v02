...
## 2026-08-21_2345 - Sharing controls S1-S12 built from the 2330 bundle

- Bundle `2026-08-21_2330_Scoreboard sharing controls - cc.zip` diffed against `docs/`: only `Win Matrix.dc.html`, `PROJECT-RECORD.md`, `decisions.md`, the brief, the design grill log and `CLAUDE.md`/`AGENTS.md` changed. Login, Scoreboard, Scoreboard Display and `styles.css` identical. All copies refreshed.
- Migration `20260821000500_sharing_controls.sql` pushed to `mzwuxjxtigapyoiqcrta`: `sharing_settings` (paused, public_token, public_depth), `share_grants.depth` gains `off`, `share_links.depth` dropped (one standing link at Summary, S3/S5), `effective_depth(owner, viewer)` is the single visibility rule and `viewer_depth` wraps it so every existing policy plus the `scores:` realtime channel inherit S7/S9/S10; `claim_share` joins at Summary with `on conflict do nothing` (the owner's Off wins over a re-click); `board_json` shared by `display_snapshot` and the new anon-callable `public_snapshot`.
- Decisions taken here, not in the design: claim never overrides an existing grant; a joiner of a paused owner gets a grant that surfaces on resume and the notice "{name} isn't sharing right now."; the public page is the glass display at `display.html?p={publicToken}` (no new screen, R6/Q28) with the COMPOSE link hidden; the public token is minted on first dialog open and stays stable across off/on; when a board vanishes the viewer's composer row is dropped by `reconcile()` and returns with defaults on restore (S9 "keeps every setting" is the owner's settings, which are kept).
- Matrix Sharing dialog ported from the prototype: Pause block with LIVE/PAUSED, invite link with Copy and Reset plus the three explainer bullets, "Who sees your board" with the pinned Public page row and OFF/SUMMARY/FULL per person, joined date, Off rows dimmed, list dimmed at 0.45 while paused. Segmented control is `.seg` in `theme.css`. The 9-hue tag palette (A5) replaces the 5 swatches.
